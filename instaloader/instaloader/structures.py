...
class Profile:
    ...
    def __init__(self, context: InstaloaderContext, node: Dict[str, Any]):
        logging.info(f'calling Profile.__init__ with args{context, node}')
    ...
    def from_username(cls, context: InstaloaderContext, username: str):
        ...
        """
        # pylint:disable=protected-access
        logging.info(f'Calling Profile.from_username with args{context, username}')
        ...
        logging.info(f'Profile.from_username returned {profile}')
        return profile
    ...
    def _asdict(self):
        logging.info(f'calling Profile._asdict with args: {self}')
        ...
        logging.info(f'Profile._asdict returned {json_node}')
        return json_node


    def _obtain_metadata(self):
        #logging.info(f'Calling Profile._obtain_metadata with args: {self}')
        print(f'Calling Profile._obtain_metadata with args')

        try:
            if not self._has_full_metadata:
                metadata = self._context.get_iphone_json(f'api/v1/users/web_profile_info/?username={self.username}',
                                                         params={})
                print(f'Profile._obtain_metadata collecting metadata')
    ...
    def _metadata(self, *keys) -> Any:
        #logging.info(f'Calling Profile._metadata with args: {keys}')
        print(f'calling Profile._metadata..')
        try:
            d = self._node
            for key in keys:
                d = d[key]
    ...
    @property
    def _iphone_struct(self) -> Dict[str, Any]:
        print(f'calling Profile._iphone_struct')
        if not self._context.iphone_support:
            raise IPhoneSupportDisabledException("iPhone support is disabled.")
        if not self._context.is_logged_in:
        ...
        print(f'Profile._iphone_struct returned {self._iphone_struct_}')
        return self._iphone_struct_
    ...
    def userid(self) -> int:
        """User ID"""
        print(f'calling Profile.userid')
        return int(self._metadata('id'))

    @property
    def username(self) -> str:
        """Profile Name"""
        print(f'calling Profile.username')
        return self._metadata('username').lower()

    
    def __repr__(self):
        print(f'calling Profile.__repr__')
        return '<Profile {} ({})>'.format(self.username, self.userid)

    
    def __eq__(self, o: object) -> bool:
        print(f'calling Profile.__eq__')
    ...
    def __hash__(self) -> int:
        print(f'calling profile.__hash__')
    ...
    def get_followers(self) -> NodeIterator['Profile']:
        ...
        """
        print('calling Profile.get_followers')
        if not self._context.is_logged_in:
            raise LoginRequiredException("Login required to get a profile's followers.")
        self._obtain_metadata()
        return NodeIterator(
    ...
    def get_followees(self) -> NodeIterator['Profile']:
        ...
        :rtype: NodeIterator[Profile]
        """
        print(f'calling Profile.get_followees')
        if not self._context.is_logged_in:
